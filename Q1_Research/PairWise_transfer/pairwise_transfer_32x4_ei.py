...
# Actually EI ranks.
def EI_ranks(input, incumbent, rl_model):
...
class RankingLossSurrogate(nn.Module):
    ...
    def observe_and_suggest(self, X_obs, y_obs, X_pen):
        ...
        scores = EI_ranks((X_obs, y_obs, X_pen), self.incumbent, self)
...
def transfer_eval(i, run):
    ...
    store_object(performance, "./" + str(run) + "/results/PAIR_OPT" + str(i))
...
if __name__ == '__main__':
    ...
    if transfer:
        print("HPO Transfer: Evaluating DE with Pair-Wise loss");
        if sys.argv[3] == "train":
