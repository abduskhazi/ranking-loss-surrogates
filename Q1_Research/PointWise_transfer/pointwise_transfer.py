...
def parallel_meta_train(args):
    nn_, meta_train_data, meta_val_data, epochs, batch_size, list_size, lr = args;
    loss_list, val_loss_list = nn_.meta_train(meta_train_data, meta_val_data, epochs, batch_size, list_size, lr)
    return loss_list, val_loss_list
...
class RankingLossSurrogate(nn.Module):
    ...
    def train_model_separate(self, meta_train_data, meta_val_data, epochs, batch_size, list_size, lr):
        with mp.Pool(len(list(self.sc))) as p:
            tuple_list = p.map(parallel_meta_train, [(nn, meta_train_data, meta_val_data, epochs, batch_size, list_size, lr) for nn in self.sc])
