...
The HPO types discussed so far do every HP optimization from scratch. These types have no prior knowledge before starting any optimization cycle.
...
\subsubsection{Types of Surrogates for BO in HPO}
...
In this chapter, we discuss the fundamental concepts one must grasp to understand the thesis work. We have already talked about the concept of ranking in Section~\ref{sec:ranklearning}.
...
The declaration of a function $s$ that maps objects in $\mathbb{A}$ to a real-valued score is given by:
$$
s : \mathbb{A} \mapsto \mathbb{R}
$$

The goal is to learn the function $s$ to predict the relevance score of any new object sampled from the set $\mathbb{A}$.
We need a loss function to use machine learning techniques to learn $s$.
If this loss function uses the concept of ranking, it is called a ranking loss function.
