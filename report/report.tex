...
It is also not possible to directly use the outputs of the ensemble. Each neural network in the ensemble outputs a separate score for any queried configuration, which may not be in a similar range.
...
Instead, throughout our thesis, we used an elementary acquisition function with ranking loss surrogates. This acquisition function selects the HP configuration with the best mean rank predicted by the ensemble of rankers.
