...
\section{Advantages of using Ranking loss surrogates}
...
Another significant advantage is that the range or the distribution of the actual scores in the metadata is irrelevant to the training procedure.
This has an inherently dampening effect on any uncertainty of the HPO objective function's output.
As the order of the known evaluations is the only relevant thing, minor variations in the objective function's output do not affect the loss's training. This advantage also allows us to manipulate the range of our surrogate's scores based on our requirements.
...
We already discussed in Section~\ref{sec:Limitations}  that the ranking loss function does not optimize the sorting function.
