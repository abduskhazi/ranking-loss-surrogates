...
A basic ranker consists of a Deep Neural Network that outputs real-valued numbers $\textbf{y}$ for a batch of inputs, say $\textbf{X}$.
...
\node[squarednode]      (FCmiddle)             [right=of FC1]        {...};
\node[squarednode]      (FCn)                      [right=of FCmiddle]        {$FC_n$};
\node[roundnodey]      (y)                             [right=of FCn] {$\textbf{y}$};
...
In addition, there is one extra but crucial step of ranking the batch of inputs in the basic ranker.
Since we use a Deep Neural Network to rank the inputs,  we may also refer to this architecture as a Deep Ranker.
...
After the training completes, the range of the outputs of the learned ML model is not vastly different from the range of the actual outputs values.
...
Hence the output range of the basic ranker can be arbitrary.
The only exception is a point-loss function called Subset Regression.
This is because subset regression directly learns the rank instead of using the output value $y$ to rank the batch of inputs.

There may be occasions where the output range of the basic ranker needs to be controlled.
...
The uncertainty estimation becomes more critical in a sequential HPO algorithm like the SMBO.
...
This is especially not good in a sequential process because selecting subsequent HP configurations depends on the previously selected HP configurations.
...
%Nodes
\node[roundnode]          (X)                                           {$\textbf{X}$};
\node[squarednode]      (DNN2)                      [right=of X]        {Deep Neural Network$_2$};
\node[squarednode]      (DNN1)                      [above=of DNN2]                    {Deep Neural Network$_1$};
\node[squarednode]      (DNN3)                      [below=of DNN2]                    {Deep Neural Network$_3$};
...
\node[squarednode]      (Rank1)                             [right=of y1] {$Rank_1$};
\node[squarednode]      (Rank2)                             [right=of y2] {$Rank_2$};
\node[squarednode]      (Rank3)                             [right=of y3] {$Rank_3$};
...
We first calculate the rank of each HP configuration in the batch as per the scorers' output.
Using these ranks, we calculate the mean rank $\mu_r$ and variance in the rank $\sigma^2_r$.
Any acquisition function that uses a ranking model needs to use this "ranking distribution" instead of the output distribution.
...
\subsubsection{Acquisition function in the Ranking space}
...
For example incumbent of EI for rank is always one... always the best.
Need for ranking support objects.
...
Lets take the listwise loss function $L_{mle}$ which is nothing by maximum log-likelihood estimation. 
In our surrogate models, we use the strictly positive increasing function $\exp$ as proposed in~\cite{listmlepaper}.
Hence our $L_{mle}$ becomes 
\begin{equation}\label{eq:overflowissue}
L_{mle} = -  \sum\limits_{j=1}^{k} \log \frac{\exp(s(\pi^*_j))}{ \sum\limits_{t=j}^k \exp(s(\pi^*_k))}
\end{equation}

Even though theoretically, the range of our basic ranker model does not need to be restricted, we have practical limitations due to implementation constraints of floating-point numbers.
Due to the exponentiation in our loss function, if the range of our basic ranker is too high,  the values in equation~\ref{eq:overflowissue} may overflow.
On the other hand, if the range is too small, they may underflow.
