...
These are also called Bayesian Neural Networks because they are based on Bayesian inference mechanism~\cite{BayesianNeuralNetworks}.
...
In Bayesian neural networks the stochasticity is added by using a stochastic activation function or representing the weights and biases of the network with a probability distribution.
Consider all weights and biases of a Bayesian neural network be represented by a parameter vector $\mathbf{\theta}$.
Let the probability distribution of the parameters $p(\mathbf{\theta})$ be a Gaussian distribution.
This is called prior probability distribution in Bayesian inference statistics.

\textbf{We learn the posterior in the following way}

Let the neural network be represented by $f$. 
The uncertainty calculation is done using the following equations

$$
\{\mathbf{\theta}_1, \mathbf{\theta}_2,  \mathbf{\theta}_3, ... \} \sim p(\mathbf{\theta})
$$
$$
\textbf{y} = \{f_{\mathbf{\theta}_1}(x),  f_{\mathbf{\theta}_2}(x),  f_{\mathbf{\theta}_3}(x) ...\}
$$

First we take a set samples from $p(\mathbf{\theta})$.
Using these samples we calculate multiple values of $f_{\theta}$ for our input $x$.
Using these multiple output values,  one can calculate the mean and the variance of the network.


\textbf{What are the disadvantages of BNNs and why we do not use it.}
