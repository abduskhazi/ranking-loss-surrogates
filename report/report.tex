...
\begin{figure}[htb]
  \centering
    ...
    \caption{Grid search and Random search in the case where 2 parameters are not equally important.  Adpated from~\cite{rshpoarticle}.}
    \label{fig:rshpofig}
\end{figure}
...
This surrogate's prediction of good-performing HP configurations is conditioned on the training meta-data. This preconditioning makes it a transfer HPO.
...
Humans do not precondition their actions on new tasks only on previous experiences.
Their preconditioning also includes the knowledge of the current task (however slight it may be).

To model this concept, one has to learn to represent and precondition knowledge from known tasks in a model.
...
Deep Sets by Zaheer et al.~\cite{deepSets} and Set Transformers by Lee et al.~\cite{setTransformer} are two exciting pieces of research we found in the literature.
...
\end{tikzpicture}
\caption{Skeleton of the architecture proposed by Zaheer et al.~\cite{deepSets}.}
...
The second objective was understanding how a non-transfer technique would work for the HP optimization problem.
...
We chose this because this gave state-of-the-art results on the HPO-B benchmark. The HPO-B benchmark is discussed later in Chapter~\ref{chap:HPOBExplanation}.
...
One question remains: What sort of architecture should our neural networks use.
...
\end{tikzpicture}
\caption{Example of an undivided DNN architecture.}
...
\end{tikzpicture}
\caption{Example of a divided DNN architecture.}
...
Even if we store the list in a data structure that stores its elements in a non-contiguous memory location (for example, a linked list), accessing the elements in the list would have $O(n)$ time complexity. This would also result in a time complexity of $O(n^2)$.
...
More precisely, the ListMLE loss is rewritten in the following form:
...
The number of ways to reach a good local minimum would increase significantly with the increase in the dimensions of the HP search space.
...
\begin{figure}[htb]
  \centering
    ...
    \caption{Loss curve obtained when cosine annealing was used.}
    \label{fig:goodFTLoss}
\end{figure}

Figure~\ref{fig:goodFTLoss} shows a finetuning loss curve obtained using cosine annealing.
This strategy reduces the problems that could prop us when using a constant learning rate. However, we realized at the later stage of the thesis that cosine annealing could not be used while finetuning the proposed model due to the issue of negative transfer learning.
...
This concatenation is passed through the deep set,  and we obtain the latent output $O_l$.
...
Since these results are not a correct yardstick to measure the ranking loss surrogate model, we used the results obtained by the FSBO implementation provided by Pineda et al. ~\cite{pineda2021hpob} in the rest of the thesis.
...
\begin{figure}[h]
  \centering
    ...
    \caption{Rank and regret graph of best performing FSBO.}
...
For example, the learning rate during meta-training and finetuning is different.
...
\begin{figure}[h]
  \centering
    ...
    \caption{Comparison of Ranking losses with regression losses (Transfer HPO case).}
...
\begin{figure}[h]
  \centering
    ...
    \caption{Comparison of listwise losses with the weighting strategy.}
...
\includegraphics[width=0.45\textwidth]{images/Q3AblationFinalRank@100}
    \caption{Critical rank graph of the listwise weighted surrogate with FSBO, GP,  and Random surrogates.}
...
\begin{figure}[h]
  \centering
    ...
    \caption{Effect of finetuning on the proposed surrogate's performance.}
...
\includegraphics[width=0.45\textwidth]{images/NonTransferSOTARank@100}
    \caption{Critical rank graphs of the proposed model and other SOTA non-transfer HPO surrogates.}
...
\includegraphics[width=0.45\textwidth]{images/TransferSOATRank@100}
    \caption{Critical rank graphs of the proposed model and SOTA transfer HPO surrogates.}
...
\begin{figure}[h]
  \centering
    ...
    \caption{Ablation of all ranking loss surrogates studied in this thesis.}
...
We compare the learned surrogates that use the Average (rank), UCB, and EI as acquisition functions with other state-of-the-art transfer HPO methods.
Figure~\ref{fig:acqusitionRanking} shows the ranked graph comparing these models.
All proposed ranking loss models in the figure~\ref{fig:acqusitionRanking} (i.e., Average-Rank, UCB-Rank, and EI-Rank) are meta-trained with weighted listwise ranking loss. Hence they are transfer HPO methods. They also use deep sets in their architecture.
\textbf{We conclude that both UCB and EI acquisition functions are superior even in the ranking space. Further, the proposed surrogate model with EI  acquisition function performs better than FSBO in all optimization steps.}

\begin{figure}[h]
  \centering
    \includegraphics[scale=0.25]{images/acqusitionRanking}
...
As the order of the known evaluations is the only relevant thing, minor variations in the objective function's output do not affect the training. This also allows us to manipulate the range of our surrogate's scores based on our requirements.
...
\begin{figure}[htb]
  \centering
    ...
    \caption{Training and validation loss curves obtained for the search space 5527.}
...
Let's take the listwise loss function $L_{mle}$ which is nothing but maximum log-likelihood estimation. 
...
\end{equation}
where $k \in \mathbb{R}$.
Note that the second parameter of the listwise loss function is scale-invariant; hence scaling the list does not affect the loss output (Section~\ref{sec:listMLE}).
...
Moreover,  a bigger list size is crucial to comprehensively learning the objective function.
