...
Hence the 2 main parts of using our optimization processes are
\begin{itemize}
...
During the training of our model we use stochastic gradient descent to train our model.
There are 2 ways to sample a batch from the given meta data.
\begin{itemize}
\item Double sampling: First sample the task then sample the meta data.
...
faster training and an availability to use a lower learning rate.
We use Adam optimizer with a learning rate of 0.001 for our model.
We do our training for a 5000 epochs and in each epoch we take 100 steps in which each step does 1 double sampling.
...
\Procedure{MetaTrain}{$s_{\theta}$,  $X_{train}$,  $y_{train}$, $epochs$}
    \For{$i < epochs$}
            \For{$j < 100$}
                ...
                \State $y_{pred} \gets s_{\theta}(B)$
                \State $\textrm{loss} \gets L_{mle}(y_{pred},  B_{y}) $
                \State $\theta \gets \theta - \alpha * \frac{\partial \textrm{loss}}{\partial \theta}$
            \EndFor
    \EndFor
\EndProcedure
\end{algorithmic}
...
THis comes into existence only when we use deep sets.
