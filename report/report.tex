...
They are inherently capable of modelling uncertainty.
Hence,  they are widely used in problems such as hyper-parameter optimization where uncertainty estimation is essential.
...
Consider a scalar random variable $X$ which is normally distributed (a.k.a Gaussian distribution) around a mean $\mu$ with a variance of $\sigma^2$.
The probability density function of $X$ is then given by the following equation 
$$
P_X(x) = \frac{1}{\sqrt[2]{2\pi}\sigma}\exp\left(- \frac{(x - \mu)^2}{2\sigma^2}\right)
$$
where $X$ represents the random variable and $x$ represents an instance of the variable\cite{GPTutorial}.
In this case,  the mean $\mu$,  variance $\sigma^2$ and any sample $x$ are all scalars.

If the random variable $\textbf{X}$ is a vector in $\mathbb{R}^d$ where ($d \in I^{+}$),  then each value in the vector can be considered as a random variable.
In this case the mean $\boldsymbol{\mu} \in \mathbb{R}^d$ whereas variance,  represented by $\Sigma \in R^{d \times d}$.
This is because the variance of all dimensions in any random valued vector $\textbf{X}$ variable should contain the following 2 components
\begin{itemize}
\item Variance of a dimension with respect to itself.
This component has $d$ values which are represented by the diagonal of the matrix $\Sigma$.
\item Variance of each dimension with respect to all other dimensions (a.k.a Covariance).
This components have $d \choose 2$ values.
These values are represented by the upper/lower triangular values in the matrix $\Sigma$.
\end{itemize}

The probability density function of a vector variable $\textbf{X} \in \mathbb{R}^d$ (multivariate normal distribution) with a mean $\boldsymbol{\mu}$ and covariance matrix $\Sigma$ is given by: \cite{MITMLBook}
...
Unfortunately,  a function sampled from $\mathbb{R}^{\infty}$ is too general and not useful by itself.

The idea of Gaussian processes is to sample smooth functions from $\mathbb{R}^{\infty}$.
Which is to say points close by in a domain of $f$ will have similar values in its range.
...
$$
This is nothing but the definition of a smooth function in terms of vector notation. 
This smoothness can be obtained if the nearby components of $f$ "vary" similarly with respect to each other.
This can be very naturally obtained by the required covariance matrix.
Hence, if we sample our function from a multivariate normal distribution with the required covariance matrix,  we will get smooth functions.
More formally,  the Gaussian process restricts the function sample space to a multivariate normal distribution space.

The similarity between 2 points in a domain can be defined by a function.
Using this function, the required covariance matrix can be populated.
This function is called the \textbf{kernel} of the Gaussian process.
The smoothness of the sampled function $f$ can be controlled by the kernel in the GP process.
More formally kernel $k$ is defined as,
$$
