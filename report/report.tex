...
\textbf{Ablation Required for all three}

\textbf{Use critical graph from HPB-O for this}

The following ablations were studied for deep ensembles.
\begin{itemize}
\item Fine tuning deep ensembles with no restart.
\item Using restart at every evaluation cycle.
\item Increasing the number of neural networks to 10.
\end{itemize}

After our analysis,  we found the following advantages of this model
\begin{itemize}
\item Being a non transfer HPO model can be applied to any continuous HPO problem.
\item Each neural network being independent can be trained in parallel.
Hence we can scale up our ensemble based on the compute power available to us.
\end{itemize}

On the negative side,  we did find some disadvantages of using Deep Ensembles:
\begin{itemize}
\item Since we do not do any meta training we are bound to (and have to) over fit to the very less data available during the optimization cycle.
\item It does not model discrete or semi discrete spaces correctly. 
