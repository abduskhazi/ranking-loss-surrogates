...
\begin{itemize}
\item Machine Learning models are very sensitive to their hyperparameters (HP).
\item Tuning hyperparameters is essential to get the most performance out of a given ML model.
\item \textbf{Aim of the thesis: Improve Hyperparameter Optimization (HPO) process.}
\item Let us define HPO first!
\end{itemize}
...
\item $\textbf{x}$ is an HP configuration.
...
\begin{itemize}
\item Various classes of HPO solutions have been proposed - Blackbox HPO,  Online HPO,  and Multi-fidelity HPO.
\item \textbf{SMBO} (Sequential Model-Based Optimization) algorithm for HPO has given good results.
\item Transfer learning has shown good results in the HPO domain due to the availability of metadata to train.
\item \textbf{Thesis target: Propose a transfer HPO surrogate for the SMBO process using the concept of ranking.}
\item The results are compared against SOTA HPO models like RGPE,  TAF,  TST, and FSBO for transfer case and GP,  BOHAMIANN, and DNGO for the non-transfer case.
\end{itemize}
...
\section{Background: Ranking Losses}

\begin{frame}
\centering
\LARGE{Background: Ranking Losses}
\end{frame}
...
\begin{itemize}
\item Ranking means ordering elements of the set $\mathbb{A}$.
...
\begin{tabular}{ | c | c | c | }
  \toprule
  Instance & Object Set & Ground Truth (Scores) \\ \midrule
% \hline \hline
...
Types of ranking:
\begin{itemize}
\item \textbf{Pointwise} - The model directly predicts the rank and not the score.  E.g.,  Subset Regression.
\item \textbf{Pairwise} - The model predicts which of the 2 inputs is better.  E.g.,  Ranknet.
\item \textbf{Listwise} - Deals with a full input set as 1 learning instance.  E.g.,  ListMLE.
\end{itemize}
...
\begin{itemize}
\item Output (or) relevance scores of input elements. These values are in the \textbf{Output Space}.
\item Ranks of the input elements. These values are in the \textbf{Ranking Space}.
\end{itemize}
...
\begin{itemize}
\item Support set, i.e., known HP configurations and their evaluations are passed through Deep Set to get a latent output.
\item The query HP configurations are concatenated with the latent output.
\item This concatenation is passed through the ensemble of \\
deep rankers.
...
\begin{frame}[t]{Meta-training the surrogate}

\begin{figure}[htb]
  \centering
    \includegraphics[scale=0.30]{images/FinalAlgorithm}
    % \caption{Negative transfer learning example.}
    \label{fig:FinalAlgorithm}
\end{figure}
...
\end{itemize}
Deep Ensemble baseline with a legend,  "DE-M10-RS",  is used as a reference.
...
\includegraphics[width=0.45\textwidth]{images/Q1AblationNonTransferRank@100}
    \caption{Loss comparisons for non-transfer HPO.}
...
\begin{figure}[h]
  \centering
    \includegraphics[scale=0.12]{images/Q1AblationTransfer} 
     \caption{Loss comparison for transfer HPO. "T" stands for transfer.}
...
\begin{itemize}
\item Inverse logarithmic weighting was used for both the transfer and non-transfer cases.
...
\begin{itemize}
\item \textbf{Non-transfer HPO results become worse.  Reason \\
is the high complexity of the model}.
\item \textbf{Transfer HPO results become very good in the initial steps}.
\end{itemize}
\end{itemize}
...
\item \textbf{Conclusion: Finetuning is required for better long-term results.}
\end{itemize}
...
\begin{itemize}
\item All previous experiments in our thesis used the average rank acquisition function.
% \item Here, we integrate UCB (Upper Confidence Bound) and EI (Expected Improvement) in the ranking space.
\item UCB (Upper Confidence Bound) is calculated using the mean and standard deviations returned from the ensemble of rankers.
\item EI (Expected Improvement) is calculated by passing the incumbent HP in both the query and the support set.
\item \textbf{Conclusion}:
\begin{itemize}
...
\textbf{Advantages}:
\begin{itemize}
\item Being a transfer HPO surrogate,  it utilises already existing metadata.
\item Sampling mechanism of listwise loss is superior.   For example,  from a set of 100 known HP configurations, there are ${100 \choose 15}$ ways to select a set of 15 items.
...
\item Study needs to be conducted to incorporate sorting functionality into the ranking losses.
% E.g., Using the PiRank model proposed by Swezey et al. ~\cite{PiRank}.
\item One could explore how to incorporate learning the mean and the variance of the rank directly in the ranking loss functions instead of explicit calculation.
...
\begin{frame}[t]{UCB and EI in the ranking space}
\begin{itemize}
\item Experiments in our thesis used the average rank acquisition function.
\item Here, we integrate UCB (Upper Confidence Bound) and EI (Expected Improvement) in the ranking space.
\item UCB is calculated using the mean and standard deviations returned from the ensemble of rankers.
\item EI is calculated by passing the incumbent HP in both the query and the support set.
...
\item Restarting the training can increase chances of reaching good optima.
\end{itemize} 
\end{itemize}
...
3 different weighting strategies were considered:
\begin{itemize}
