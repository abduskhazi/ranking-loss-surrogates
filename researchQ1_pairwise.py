...
from rankNet import rankNet as listMLE
...
def listMLE_(y_pred, y_true, eps=DEFAULT_EPS, padded_value_indicator=PADDED_Y_VALUE):
    """
...
def test_toy_problem():
    ...
    for _ in range(epochs):
        ...
        train_data = get_training_data((1, 10, 100))
    ...
    r1 = 1 
    r2 = 100
    get_validation_data = lambda x: (r1 - r2) * torch.rand(x + (1,)) + r2

    for i in range(1000):

        # Changing the size for it to pass through our scorer
        train_data = get_validation_data((100,))
        prediction = sc(train_data)
...
class RankingLossSurrogate(nn.Module):
    def __init__(self, input_dim, file_name=None):
        ...
        self.save_folder = "./save/pair_rlsurrogates_deepset_16_5000_uncertainty/"
