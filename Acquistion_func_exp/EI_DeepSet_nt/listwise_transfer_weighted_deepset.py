...
class RankingLossSurrogate(nn.Module):
    ...
    def observe_and_suggest(self, X_obs, y_obs, X_pen):
        ...
        restarted_model = RankingLossSurrogate(input_dim=self.input_dim, ssid=self.ssid)
        restarted_model.fine_tune_together(X_obs, y_obs, epochs=1000, lr=0.001)
        scores = average_ranks_deep_set((X_obs, y_obs, X_pen), self.incumbent, restarted_model)
...
def evaluate_transfer_rl(hpob_hdlr, keys_to_evaluate):
    ...
    for key in keys_to_evaluate:
        ...
        method = RankingLossSurrogate(input_dim=input_dim, ssid=search_space, load=False)
